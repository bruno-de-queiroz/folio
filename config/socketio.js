module.exports = function(app,config,port){
	...
	io.set("authorization", passio.authorize({
		key: config.app.key					//the cookie where express (or connect) stores its session id.
		, secret: config.app.secret				//the session secret to parse the cookie
		, store: config.app.session	//the session store that express uses
